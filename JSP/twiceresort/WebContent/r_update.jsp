...
<body>
	...
	<div class="container">
		...
		<table border="1">
			...
			<tr>
				<td>전화번호</td>
				<td style="font-weight: bold;"><%=telnum%></td>
			</tr>
			<tr>
				<td>입금자명</td>
		...
		<br>
		<br> <input type=button style="WIDTH: 200pt; HEIGHT: 30pt"
			value="홈으로 가기" Onclick="location.href='newindex.jsp'"> <input
			type=button style="WIDTH: 200pt; HEIGHT: 30pt" value="관리자 페이지로 가기"
		...
		<%
			...
			dto.setName(name); //dto객체에 name set.
			dto.setDate(date); //dto객체에 date set.
			dto.setRoom(introom); //dto객체에 introom set.
			dto.setAddr(addr); //dto객체에 addr set.
			dto.setTelnum(telnum); //dto객체에 telnum set.
			dto.setIn_name(iname); //dto객체에 iname set.
			dto.setComment(comment); //dto객체에 comment set.
			dto.setWrite_date(now); //dto객체에 now set.
			dto.setProcessing(intprocessing); //dto객체에 intprocessing set.
			dto.setfirstDate(firstDate); //dto객체에 firstDate set.
			dto.setfirstRoom(intfirstRoom); //dto객체에 intfirstRoom set.

			boolean r = dao.update(dto);

			if (r) {
				out.println("<br>수정 성공.");
			} else {
				out.println("<br>수정 실패. 예약 사항이 중복됐는지 확인해주세요.");
			}
		%>
	</div>
	<br>
